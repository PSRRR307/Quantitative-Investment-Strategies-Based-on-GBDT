...
class QlibTopKStrategy(BaseStrategy):
    ...
    def to_signal(self, data: pd.DataFrame, position: dict, cash_available: float = None):
        ...
        n_k = int(len(data) * self.k + 0.5) or 1                # 四舍五入计算选股数(至少一只)(3)
        ...
        if self.equal_weight:
            data_buy["predict"] = cash_available / n_k
        else:
            # 以预计收益比率（预估收益 / 开盘价格）为权重
            data_buy["expected_return_ratio"] = data_buy["Predict_close_price"] / data_buy["Open_price"]
            total_return_ratio = data_buy["expected_return_ratio"].sum()
            data_buy["predict"] = data_buy["expected_return_ratio"] / total_return_ratio * cash_available
...
def process_folder(folder_path: str, strategy: BaseStrategy, initial_cash: float = 100000.0):
    ...
    for file_name in excel_files:
        ...
        try:
            ...
            print(f"当天利润: {daily_profit}, 累计利润: {cumulative_profit}, 收益比: {cash_available / initial_cash:.4f}")
